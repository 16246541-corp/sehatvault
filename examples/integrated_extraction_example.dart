...
void generateHealthReport(Map<String, dynamic> evaluation) {
  ...
  for (var result in [...criticalHigh, ...criticalLow, ...borderlineHigh, ...borderlineLow]) {
    ...
    if (testName.contains('hemoglobin') && status == 'low') {
      ...
      recommendations.add('Consider iron-rich foods (spinach, red meat, lentils) and consult your doctor about iron supplementation');
    }
    
    if (testName.contains('glucose') && status == 'high') {
      insights.add('Elevated glucose suggests impaired glucose tolerance or pre-diabetes');
      recommendations.add('Reduce sugar and refined carbohydrate intake, increase physical activity, and monitor blood sugar regularly');
    }
    
    if (testName.contains('hba1c') && status == 'high') {
      insights.add('Elevated HbA1c indicates poor blood sugar control over the past 2-3 months');
      recommendations.add('Work with your healthcare provider to develop a diabetes management plan');
    }
    
    if (testName.contains('cholesterol') && status == 'high') {
      insights.add('High cholesterol increases cardiovascular disease risk');
      recommendations.add('Adopt a heart-healthy diet (reduce saturated fats, increase omega-3s), exercise regularly, and discuss statin therapy with your doctor');
    }
    
    if (testName.contains('ldl') && status == 'high') {
      insights.add('Elevated LDL ("bad cholesterol") contributes to arterial plaque buildup');
      recommendations.add('Focus on soluble fiber (oats, beans), plant sterols, and regular aerobic exercise');
    }
    
    if (testName.contains('triglycerides') && status == 'high') {
      insights.add('High triglycerides may indicate metabolic syndrome');
      recommendations.add('Limit alcohol, reduce simple sugars, and increase omega-3 fatty acids (fish, flaxseed)');
    }
    
    if (testName.contains('tsh') && status == 'high') {
      insights.add('Elevated TSH suggests hypothyroidism (underactive thyroid)');
      recommendations.add('Consult an endocrinologist for thyroid function evaluation and potential levothyroxine therapy');
    }
    
    if (testName.contains('vitamin d') && status == 'low') {
      insights.add('Vitamin D deficiency can affect bone health and immune function');
      recommendations.add('Increase sun exposure (15-20 min daily), consume vitamin D-rich foods (fatty fish, fortified milk), and consider supplementation (1000-2000 IU daily)');
    }
  }
