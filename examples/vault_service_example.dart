...
void main() async {
  ...
  try {
    ...
    print(
        '  Text Length: ${healthRecord.metadata?['textLength']} characters\n');
  } catch (e) {
    print('✗ Error: $e\n');
  }
  ...
  try {
    ...
    if (allDocs.isNotEmpty) {
      ...
      if (firstDoc.extraction != null) {
        ...
        print(
            '    - Text Length: ${firstDoc.extraction!.extractedText.length}');
        print(
            '    - Structured Fields: ${firstDoc.extraction!.structuredData.keys.join(", ")}');
  ...
  try {
    ...
    final docsWithExtractions =
        allDocs.where((d) => d.extraction != null).length;
    ...
    if (docsWithExtractions > 0) {
      final avgConfidence = allDocs
          ...
          docsWithExtractions;
      print(
          '  Average Confidence Score: ${(avgConfidence * 100).toStringAsFixed(1)}%');
    }
    print('');
  } catch (e) {
    print('✗ Error: $e\n');
