...
class DataExtractionService {
  ...
  static List<Map<String, String>> _extractLabValues(String text) {
    ...
    final commonLabTerms = [
      'glucose', 'hemoglobin', 'hba1c', 'cholesterol', 'ldl', 'hdl', 
      'triglycerides', 'creatinine', 'urea', 'bilirubin', 'albumin', 
      'protein', 'sodium', 'potassium', 'chloride', 'calcium', 'wbc', 
      'rbc', 'platelets', 'tsh', 't3', 't4', 'vitamin', 'iron'
    ];

    final commonUnits = [
      'g/dl', 'mg/dl', 'mmol/l', '%', 'u/l', 'mcg', 'ml', 'kg', 'iu', 'mEq/L'
    ];
    ...
    for (var match in labRegex.allMatches(text)) {
      ...
      final isMedicalTerm = commonLabTerms.any((term) => field.toLowerCase().contains(term));
      final hasUnit = unit.isNotEmpty && commonUnits.any((u) => unit.toLowerCase().contains(u.toLowerCase()));
  ...
  static List<Map<String, String>> _extractMedications(String text) {
    ...
    for (var match in medRegex.allMatches(text)) {
      ...
      final commonWords = ['time', 'date', 'test', 'result', 'page', 'phone', 'name'];
  ...
  static List<Map<String, String>> _extractVitals(String text) {
    ...
    // BP: 120/80
    final bpRegex = RegExp(r'\b(BP|Blood Pressure)[:\s]*(\d{2,3}/\d{2,3})\b', caseSensitive: false);
    for (var match in bpRegex.allMatches(text)) {
      vitals.add({'name': 'Blood Pressure', 'value': match.group(2)!});
    }

    // HR/Pulse: 72 bpm
    final hrRegex = RegExp(r'\b(HR|Pulse|Heart Rate)[:\s]*(\d{2,3})\s*(?:bpm)?\b', caseSensitive: false);
    for (var match in hrRegex.allMatches(text)) {
      vitals.add({'name': 'Heart Rate', 'value': match.group(2)!, 'unit': 'bpm'});
    }

    // Temp: 98.6 F or 37 C or 98.6
    final tempRegex = RegExp(r'\b(?:Temp|Temperature)[:\s]*(\d{2,3}(?:\.\d+)?)\s*([FC])\b', caseSensitive: false);
    for (var match in tempRegex.allMatches(text)) {
      vitals.add({'name': 'Temperature', 'value': match.group(1)!, 'unit': match.group(2)!});
    }
