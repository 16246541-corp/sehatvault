...
class LLMEngine {
  ...
  Future<void> initialize(ModelOption model, {bool forceReload = false}) async {
    ...
    try {
      ...
      // Check for simulation mode (dummy model data)
      final firstBytes = await modelFile.openRead(0, 20).first;
      final contentPrefix = String.fromCharCodes(firstBytes);
      if (contentPrefix.startsWith('DUMMY_MODEL_DATA')) {
        SecureLogger.log(
            "Simulation Mode: Dummy model detected, skipping native initialization.");
        _isSimulationMode = true;
        _llama = null; // We'll handle this in generate()
        _currentModel = model;
  ...
  Future<void> _verifyChecksum(ModelOption model) async {
    ...
    // Check if it's a dummy file for simulation
    final firstBytes = await file.openRead(0, 20).first;
    final contentPrefix = String.fromCharCodes(firstBytes);
    if (contentPrefix.startsWith('DUMMY_MODEL_DATA')) {
      SecureLogger.log("Simulation Mode: Bypassing checksum for dummy model.");
      return;
    }
  ...
  Stream<String> generate(String prompt) async* {
    if (_llama == null && !_isSimulationMode) {
    ...
    try {
      if (_isSimulationMode) {
        final dummyResponse =
            "This is a simulated response from the AI engine. In a real environment, this would be generated by the ${_currentModel?.name ?? 'selected'} model. Your device and security configurations are correctly set up for local AI processing.";
