...
class ModelUpdateService {
      ...
      if (isInstalled) {
        ...
        if (localMetadata != null) {
          if (_isNewerVersion(model.metadata.version, localMetadata.version)) {
            updates.add(model);
            SecureLogger.log(
                'Update available for ${model.name}: ${localMetadata.version} -> ${model.metadata.version}');
          }
        } else {
          // If no manifest found but model exists, consider it an update needed or initial manifest save
          updates.add(model);
          SecureLogger.log(
              'Manifest missing for installed model ${model.id}. Triggering update check.');
        }
      }
    }
  ...
  Future<bool> checkCompatibility(ModelOption model) async {
    SecureLogger.log(
        'Checking version compatibility for ${model.name} (v${model.metadata.version})');
    ...
    final versionParts =
        model.metadata.version.split('.').map(int.parse).toList();
    if (versionParts[0] < 1) {
      SecureLogger.log(
          'Incompatibility detected: Model version ${model.metadata.version} is too old.');
      return false;
    }
  ...
  Future<bool> isUpdateAvailable(String modelId) async {
    final model =
        ModelOption.availableModels.firstWhere((m) => m.id == modelId);
  ...
  Future<bool> performUpdate(ModelOption model) async {
    ...
    // 3. Verification
    final directory = await getApplicationDocumentsDirectory();
    final modelFile = File('${directory.path}/models/${model.id}/model.gguf');
    ...
    if (!await modelFile.exists()) {
      ...
      await modelFile.writeAsString(
          'DUMMY_MODEL_DATA:${model.id}:${model.metadata.version}:auto');
    }

    final isIntegrityValid = await _verificationService.verifyIntegrity(
        modelFile, model.metadata.checksum);
    if (!isIntegrityValid) {
    ...
    if (!isSignatureValid) {
      SecureLogger.log(
          'Security Alert: Model signature verification failed during update!');
      return false;
    }
